...
var messages = [1793, 1794, 1795, 1796, 1797].map((id) => ({
  id,
  dir: "OUT",
  data: Buffer.from([0, 0, 0, 0, 0, 0, 0, 0]),
  // <-- personalizza il payload
  msgType: {
    idType: import_ECB2.CAN_ID_TYPE.STANDARD,
    // ID standard (11 bit), 0x701-0x705 rientrano nel range
    remote: false,
    canfd: false,
    brs: false
  }
  ...
  timer = setInterval(async () => {
    try {
      for (const msg of messages) {
        await (0, import_ECB2.output)(msg);
      }
    } catch (error) {
